  ...
  return (
    <div className="space-y-5 pb-24">
      ...
      <div className="bg-white rounded-3xl p-5 shadow-editorial border border-[#073b4c]/5 relative overflow-hidden">
        ...
        <span className="inline-block px-3 py-1 bg-[#ef476f]/15 text-[#ef476f] text-[11px] font-bold rounded-full mb-2.5">
      ...
      <div className="space-y-4">
        {decisions.map((dec) => {
          ...
          return (
            <div
              key={dec.id}
              className={`bg-white rounded-3xl p-5 shadow-editorial border transition-all duration-300 space-y-3.5 ${
                isResolved ? 'border-[#06d6a0]/40 bg-[#06d6a0]/5' : 'border-[#073b4c]/5 hover:shadow-editorial-hover'
              }`}
            >
              {/* Header de Alerta */}
              <div className="flex items-start justify-between gap-3">
                <span className={`inline-flex items-center gap-1.5 px-3 py-1 text-[11px] font-bold rounded-full border ${badge.bg} ${badge.text} ${badge.border}`}>
              ...
              {/* Título */}
              <h3 className={`text-sm font-bold text-[#073b4c] leading-snug ${isResolved ? 'line-through opacity-70' : ''}`}>
                {dec.titulo}
              </h3>
              ...
              <div className="bg-[#118ab2]/8 p-3.5 rounded-2xl border border-[#118ab2]/20">
                <h4 className="text-[11px] font-bold text-[#118ab2] uppercase tracking-wider mb-1 flex items-center gap-1.5">
              ...
              {/* Impacto Estimado */}
              <div className="flex items-center justify-between text-xs font-semibold text-[#06d6a0] bg-[#06d6a0]/10 px-3 py-2 rounded-xl border border-[#06d6a0]/20">
                ...
                <button
                  ...
                  className="text-[11px] font-bold text-[#118ab2] hover:underline flex items-center gap-1"
                >
                  <FileText className="w-3.5 h-3.5" /> Ver Evidencia
                </button>
              </div>

              {/* Evidencia y Citas trazables */}
              <div className="pt-2 border-t border-[#073b4c]/5 space-y-2">
                <span className="text-[10px] font-bold text-[#073b4c]/50 uppercase tracking-wider block">
                ...
                <div className="flex flex-wrap gap-2">
                  {dec.archivos_citados.map((doc, idx) => (
                    <button
                      ...
                      className="inline-flex items-center gap-1 text-[10px] font-semibold bg-[#FAF9F5] hover:bg-[#ffd166]/30 text-[#073b4c] px-2.5 py-1 rounded-xl border border-[#073b4c]/10 transition-colors shadow-chip"
                    >
              ...
              <div className="pt-2 flex items-center justify-between gap-2 border-t border-[#073b4c]/5">
                <div className="flex items-center gap-1 text-[11px] text-[#073b4c]/60 font-medium">
                  <UserCheck className="w-3.5 h-3.5 text-[#118ab2]" /> {dec.responsable_accion}
                </div>
