...
export const DataAuditView: React.FC<DataAuditViewProps> = ({ audits }) => {
  ...
  return (
    <div className="space-y-4 pb-20">
      ...
      {/* Filter Tabs */}
      <div className="flex items-center gap-1 bg-white p-1 rounded-2xl border border-[#073b4c]/5 shadow-sm text-[11px] font-semibold max-w-md">
        <button
      ...
      {/* Tarjetas de Auditoría por Documento (Grid 1 o 2 cols) */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        {filteredAudits.map((audit) => (
          <div
            key={audit.asset_id}
            onClick={() => setSelectedAudit(audit)}
            className="bg-white rounded-3xl p-4 shadow-editorial border border-[#073b4c]/5 hover:shadow-editorial-hover transition-all duration-200 cursor-pointer flex flex-col justify-between space-y-3"
          >
      ...
      {selectedAudit && (
        <div className="fixed inset-0 z-50 bg-[#073b4c]/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl p-4 max-w-sm w-full shadow-editorial border border-[#073b4c]/10 animate-in fade-in zoom-in duration-200">
